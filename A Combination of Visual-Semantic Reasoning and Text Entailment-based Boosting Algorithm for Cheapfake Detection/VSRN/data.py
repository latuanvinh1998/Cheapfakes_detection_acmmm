...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, f_json, vocab, idx, opt):
        self.vocab = vocab
        # loc = data_path + '/'
        self.images = np.load(data_path + str(idx) + '.npy')
        # Captions
        self.captions = []
        token_caption = []
        len_ = self.images.shape[0]


        for i in range(len_):
            idx_ = idx*10000 + i
            self.captions.append(f_json[idx_]["articles"][0]['caption_modified'].strip())
            tokens = nltk.tokenize.word_tokenize(str(f_json[idx_]["articles"][0]['caption_modified'].strip()).lower())
            token_caption.append(tokens)        
