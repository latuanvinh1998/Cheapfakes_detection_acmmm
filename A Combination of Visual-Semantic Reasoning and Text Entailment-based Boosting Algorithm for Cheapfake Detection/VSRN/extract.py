...
path = '/drive/'
f = open(path + 'cosmos_anns/test_data.json')
labels = []
...
# np.save('/drive/test_task_2.npy', features)

for i in range(len(labels)):
	print labels[i]['img_local_path']
	path_ = path + labels[i]['img_local_path']
	result = get_detections_from_im(net, path_, 0)
	features.append(result['features'])

features = np.asarray(features)
np.save('/drive/test_task_1.npy', features)

# data = np.load('/drive/Data_Npy/Train/train_16.npy')

# for i in range(1000):
# 	idx = randrange(160000, 161754)
# 	print labels[idx]['img_local_path']
# 	path_ = path + labels[idx]['img_local_path']
# 	result = get_detections_from_im(net, path_, 0)
# 	print result['features'] == data[idx - 160000,:,:]
